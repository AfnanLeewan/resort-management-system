          ...
          <div className="flex justify-between items-center mb-6 shrink-0">
            ...
            <div className="flex items-center gap-6">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all ${isMultiSelectMode
                                          ...
                                          {(() => {
                                            ...
                                            return (
                                              <button
                                                ...
                                                className={`flex-1 py-4 rounded-xl font-bold transition-all shadow-lg flex items-center justify-center gap-2 ${isPaid
...
function BookingModal({ rooms, onClose, onSuccess, currentUser, initialDate }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const booking: Booking = {
      ...
      roomGuests: Object.keys(roomGuests).length > 0 ? roomGuests : undefined,
      createdAt: new Date().toISOString(),
      ...
      <div className="bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl scale-100 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            ...
            {isGroup && (
              <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl mt-4 space-y-4">
                <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                  <UserIcon className="w-4 h-4 text-cyan-500" />
                  ข้อมูลผู้เข้าพักแต่ละห้อง (ตัวเลือกเสริม)
                </h4>
                <p className="text-xs text-slate-500">ระบุชื่อผู้เข้าพัก สำหรับการเช็คอินแยกห้อง หรือเว้นว่างไว้หากไม่ทราบ</p>

                <div className="space-y-3 mt-3">
                  {rooms.map((room: any) => (
                    <div key={room.id} className="p-3 bg-white border border-slate-100 rounded-lg flex flex-col gap-2">
                      <span className="text-xs font-bold text-slate-700 bg-slate-100 px-2 py-1 rounded w-fit">
                        {room.label}
                      </span>
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          placeholder="ชื่อ-นามสกุล..."
                          className="text-sm p-2 bg-slate-50 border border-slate-200 rounded focus:ring-1 focus:ring-cyan-500 outline-none w-full"
                          value={roomGuests[room.id]?.name || ''}
                          onChange={(e) => setRoomGuests(prev => ({
                            ...prev,
                        ...
                        <input
                          ...
                          className="text-sm p-2 bg-slate-50 border border-slate-200 rounded focus:ring-1 focus:ring-cyan-500 outline-none w-full"
                          value={roomGuests[room.id]?.phone || ''}
                          onChange={(e) => setRoomGuests(prev => ({
                            ...prev,
                            [room.id]: { ...prev[room.id], phone: e.target.value }
                          }))}
                        />
                      </div>
                    </div>
